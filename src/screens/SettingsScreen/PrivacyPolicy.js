...
const PrivacyPolicy = ({navigation}) => {
  ...
  const getData = async () => {
    ...
    try {
      const res = await config.get('/get-privacy', {
        headers: {
  ...
  return (
    <SafeAreaView style={styles.container}>
       <MainHeader navigation={navigation} backButton={true} logo={false} heading={'Privacy Policy'} />
       <ScrollView>
       <View style={{ padding: moderateScale(5, 0.1),marginTop:moderateScale(40,0.1)}}>
        {responseData?.Privacy?.description && (
